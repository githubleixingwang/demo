...
/**
 * @author: lxw
 * @date: 2023-06-01  10:39
 */
@RestController
@Slf4j
public class ExcelBigDataDownload {

    public static final String FILE_NAME = "E:" + File.separator + "test_" + System.currentTimeMillis() + ".xlsx";
    // 每个 sheet 写入的数据
     ...
     * @return list 集合
     */
    private List<ExcelBean> getDate(Long count) {
        log.info("开始生成数据");
        Date date = new Date();
    ...
    public void downloadExcelByApi(Long count, HttpServletResponse response) throws IOException {
        ...
        log.info("下载excel名称={}", fileName);
        long startTime = System.currentTimeMillis();
        // 直接调用api
        List<ExcelBean> date = getDate(count);
        EasyExcel.write(response.getOutputStream(), ExcelBean.class).sheet("test").doWrite(date);
        log.info("下载excel结束,数据量={}，耗时={}ms", date.size(), System.currentTimeMillis() - startTime);
    }
    ...
    public void writeExcelByMulSheet(Long count, HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.ms-excel");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("测试", "UTF-8");
        response.setHeader("Content-disposition", "attachment;filename=" + fileName + ".xlsx");
        ...
        // 指定写入的文件
        ExcelWriter excelWriter = EasyExcel.write(response.getOutputStream(), ExcelBean.class).build();
    ...
    public void writeExcelByMulWrite(Long count,HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.ms-excel");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("测试", "UTF-8");
        response.setHeader("Content-disposition", "attachment;filename=" + fileName + ".xlsx");
        ...
        // 获取数据
        List<ExcelBean> date = getDate(count);
        ExcelWriter excelWrite = EasyExcel.write(response.getOutputStream(), ExcelBean.class).build();
        ...
        log.info("下载excel结束,总数据量={}，耗时={}ms", date.size(), System.currentTimeMillis() - startTime);
    }
    ...
    public void writeExcelByMulSheetAndWriteChange(Long count, HttpServletResponse response) throws IOException {
        response.setContentType("application/vnd.ms-excel");
        response.setCharacterEncoding("utf-8");
        String fileName = URLEncoder.encode("测试", "UTF-8");
        response.setHeader("Content-disposition", "attachment;filename=" + fileName + ".xlsx");
        ...
        for (int i = 0; i < sheetNum; i++) {
            ...
            for (int j = 0; j < writeNum; j++) {
                ...
                excelWriter.write(date.subList(startNum, endNum), writeSheet);
                log.info("下载sheet={}，数据量={}-{}={}，耗时={}", sheetName, endNum, startNum, endNum - startNum, System.currentTimeMillis() - l);
            }
        }
        // 需要放入 finally 中
        if (excelWriter != null) {
